  ...
  providers: [
    ...
    provideAnimationsAsync(),
    {
      ...
      useFactory: () => {
        ...
        const token = localStorage.getItem('accessToken');
        const wsLink = new GraphQLWsLink(createClient({ url: environment.graphqlWsEndpoint, connectionParams: { serviceName: 'notification', headers: token ? { Authorization: 'Bearer ' + token } : {} } }));
        const link = split(
          ({ query }) => {
